...
import styles from "./search-client.module.scss";
import SearchItemAlbum from "../search-item-album/search-item-album";
import SearchItemTrack from "../search-item-track/search-item-track";
    ...
    <div className={styles.search}>
      ...
      {searchType === "all" && (
        <div className={styles.search__type_all}>
          <div className="albums flex flex-col">
            ...
            >
              {searchData.albums.items
                ...
                .map((album: SupabaseAlbum) => (
                  <SearchItemAlbum key={album.item_id} data={album} />
                ))}
            </div>
          </div>
          ...
          <div className="albums flex flex-col">
            ...
            {searchData.tracks.items.slice(0, 5).map((track: SupabaseTrack) => (
              <SearchItemTrack key={track.item_id} data={track} />
            ))}
          </div>
        </div>
      )}
      ...
      {searchType === "albums" && (
        <div className="albums flex flex-col">
          ...
          <div className={`${styles.search__albums_items} flex flex-col`}>
            {searchData.albums.items.map((album: SupabaseAlbum) => (
              <SearchItemAlbum key={album.item_id} data={album} />
            ))}
          </div>
        </div>
      ...
      {searchType === "tracks" && (
        <div className="albums flex flex-col">
          ...
          {searchData.tracks.items.map((track: SupabaseTrack) => (
            <SearchItemTrack key={track.item_id} data={track} />
          ))}
        </div>
      )}
